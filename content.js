function getElements(isStart) {
  ...
  // I'm going to refactor this function into, getSelectedInfo and subtractString
...
function getSelectedInfo() {
  ...
  if (window.getSelection) {
    console.log("50")
    let sel = window.getSelection(); // sel is a selection not a node
    let selNode = sel.anchorNode // The node where the selection begins
    text = sel.toString(); // this is the highlighted text
    let totalNodeText = sel.baseNode.textContent // This is the total text of the node highlighted
    // Here I need to remove the highlighted text from the total node text, (subtractString)

    let subtracted = subtractString(totalNodeText, text);
    let subtractedTotalNodeText = subtracted[0];
    let positionToReinsert = subtracted[1];
    // then translate that text
    let translated = convertText(text, "Heiltsuk Duolos", "Unicode");
    ...
    // then put that text back into the DOM.... but where exactly?
    // all we want to do is replace the old (selected) node with the new node we just created.
    // So we need its parentnode, and we need to know which child it selected
    console.log(`${subtractedTotalNodeText}  ---------  ${translated}`)
    ...
    console.log(`window.getselection is falsy`)
  }
  return "Text should be translated";
}
...
// what are input and output in this function?
